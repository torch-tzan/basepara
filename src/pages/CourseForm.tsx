...
// CourseForm component for creating and editing public courses
const CourseForm = () => {
  ...
  const { actions, courseCategories, actionCategories, getCourseById, getActionsByIds, addCourse, updateCourse } = useTrainingData();
  const { logCourseCreated, logCourseUpdated } = useAuditLog();
  
  // 課程分類選項（從 Context 取得已排序的分類）
  const courseCategoryOptions = courseCategories.map((cat) => ({
    value: cat.name,
    label: cat.name,
  }));
  ...
  }, [courseId, copyFromId, isCopyMode, getCourseById, getActionsByIds]);
  ...
  const hasChanges = useCallback(() => {
    ...
    // 檢查動作參數是否有變更
    const paramsChanged = selectedActions.some((action, index) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEditMode && courseId) {
        ...
        // Log course update
        await logCourseUpdated(courseId, name.trim(), {
          category,
        ...
        // Log course creation
        await logCourseCreated(newCourse.id, name.trim());
  ...
  return (
    <AppLayout
      title={isEditMode ? "編輯課程" : isCopyMode ? "複製課程" : "新增課程"}
      headerAction={
